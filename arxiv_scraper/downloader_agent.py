...
class MCPEnhancedDownloaderAgent:
    ...
    async def initialize(self) -> bool:
        ...
        if connection_result:
            ...
            self.logger.warning(
                "Fetch MCP server not available, using fallback downloader"
            )
            return False

    async def download_pdf(
        self, url: str, paper_id: str = None, verify_integrity: bool = True
    ) -> DownloadResult:
        """
        ...
        try:
            ...
            if local_path.exists():
                ...
                result = DownloadResult(
                    ...
                    checksum=(
                        await self._calculate_checksum(local_path)
                        if verify_integrity
                        else None
                    ),
                    download_time=0,
            ...
            if mcp_manager.is_server_connected("fetch"):
                result = await self._download_with_mcp(
                    url, local_path, verify_integrity
                )
            else:
                result = await self._download_fallback(
                    url, local_path, verify_integrity
                )
            ...
            if result.success:
                self.logger.info(
                    f"Downloaded PDF: {url} -> {result.local_path} ({result.file_size} bytes)"
                )
    ...
    async def download_batch(
        self, pdf_urls: List[str], max_concurrent: int = 5
    ) -> List[DownloadResult]:
        """
        ...
        self.logger.info(
            f"Batch download completed: {successful}/{len(pdf_urls)} successful"
        )
    ...
    async def _download_with_mcp(
        self, url: str, local_path: Path, verify_integrity: bool
    ) -> DownloadResult:
        ...
        try:
            response = await mcp_manager.call_tool(
                "fetch",
                "fetch_url",
                {"url": url, "save_to_file": str(local_path), "timeout": 60},
            )
            ...
            if response.get("success"):
                ...
                return DownloadResult(
                    url=url, local_path=None, success=False, error_message=error_msg
                )
        ...
        for file_path in self.storage_path.rglob("*.pdf"):
            if file_path.is_file():
                try:
                    ...
                    self.logger.error(
                        f"Checksum calculation failed for {file_path}: {e}"
                    )
    ...
    async def get_download_stats(self) -> Dict[str, Any]:
        ...
        if self.download_history:
            avg_size = sum(
                r.file_size for r in self.download_history if r.success
            ) / max(successful, 1)
            avg_time = sum(r.download_time for r in self.download_history) / len(
                self.download_history
            )
        else:
            avg_size = 0
