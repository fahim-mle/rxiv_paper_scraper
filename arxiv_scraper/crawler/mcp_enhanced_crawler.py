...
class MCPEnhancedCrawlerAgent:
    ...
    async def initialize(self) -> bool:
        ...
        self.logger.info(
            f"Connected to {success_count}/{len(required_servers)} required MCP servers"
        )

        return success_count >= 2  # At least arxiv and one other server

    async def crawl_arxiv(
        self, query: str = "cat:cs.AI", max_results: int = 100, date_range: tuple = None
    ) -> CrawlResult:
        """
        ...
        try:
            ...
            self.logger.info(
                f"Crawled {len(papers)} papers from arXiv with query: {query}"
            )

            return CrawlResult(source="arxiv", papers=papers, success=True)

        except Exception as e:
            self.logger.error(f"arXiv crawl failed: {e}")
            return CrawlResult(
                source="arxiv", papers=[], success=False, error_message=str(e)
            )
        ...
        for url in urls:
            try:
                ...
                nav_response = await mcp_manager.call_tool(
                    "playwright", "navigate", {"url": url, "wait_for": "networkidle"}
                )
                ...
                if nav_response.get("success"):
                    ...
                    if extract_response.get("content"):
                        paper_data = self._parse_web_content(
                            extract_response["content"], url
                        )
    ...
    async def fetch_content_urls(self, urls: List[str]) -> CrawlResult:
        ...
        for url in urls:
            try:
                ...
                response = await mcp_manager.call_tool(
                    "fetch", "fetch_content", {"url": url, "format": "markdown"}
                )
        ...
        return CrawlResult(
            source="fetch", papers=papers, success=success, error_message=error_message
        )
    ...
    async def extract_paper_urls(self, source_urls: List[str]) -> List[str]:
        ...
        for url in source_urls:
            try:
                if mcp_manager.is_server_connected("playwright"):
                    ...
                    await mcp_manager.call_tool("playwright", "navigate", {"url": url})
        ...
        self.logger.info(
            f"Extracted {len(valid_urls)} paper URLs from {len(source_urls)} source pages"
        )
    ...
    def _is_valid_paper_url(self, url: str) -> bool:
        ...
        paper_indicators = ["arxiv.org", ".pdf", "paper", "publication", "doi.org"]
    ...
    async def get_server_status(self) -> Dict[str, str]:
        ...
        return {
            server: mcp_manager.get_server_status(server).value for server in servers
        }
